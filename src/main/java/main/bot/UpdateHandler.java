...
import main.bot.statehandlers.HandlerGetter;
...
public class UpdateHandler {
  ...
  /*  public SendMessage handleUpdate(Update incomingUpdate) {
        Update update = incomingUpdate;
    ...
    public SendMessage handleUpdate(Update incomingUpdate) {
        Message message = incomingUpdate.getMessage();
        long userId = message.getFrom().getId();
        String messageText = message.getText();
        BotState botState;
        ...
        switch (messageText) {
            ...
            case "/menu":
                ...
                break;
            case "/find":
                if (isUserAllowedToCreateNewRequest(userId)) {
                    botState = BotState.NEW_SEARCH;
                } else {
                    botState = BotState.TOO_MUCH_REQUESTS_FOR_USER;
                }
                break;
            case "/myrequests":
                botState = BotState.MY_REQUESTS;
                break;
            default:
                botState = BotCache.getBotState(userId);
                break;
        }
        BotCache.saveToCache(userId, botState);
        Handler handler = handlerGetter.getHandler(botState);
        replyMessage = handler.handleUpdate(incomingUpdate);
        return replyMessage;
    }


    private boolean isUserAllowedToCreateNewRequest(long userId) {
        return getNumberOfExistingTasksForUser(userId) < maximumAmountOfTasksPerUser;
    }

    private int getNumberOfExistingTasksForUser(long userId) {
        List<TaskEntity> tasks = (List<TaskEntity>) ticketEntityRepository.findUserRequests(userId);
